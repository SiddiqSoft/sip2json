...
| **JSON Metaphor & Schema** | Wire Model | Specification for the compact single-character key schema (`s`, `h`, `b`, `meta`) mapping SIP frames to JSON. | [JSON Schema](../features/json_schema.md) |
| **SDP Support** | Media Descriptors | Native Session Description Protocol parsing and serialization into structured `/b/sdp` objects. | [SDP Support](../features/sdp.md) |
